# BaseClass is a class for conditionalDistributions sample() and log_prob() methods,
# e.g torch.distributions.Distribution,
import torch
...
class ScoreDistribution(Distribution):
    ...
    def __init__(
        ...
        self.noise_distribution = Normal(
            loc=score_estimator.mean * torch.ones(event_shape),
            scale=score_estimator.std * torch.ones(event_shape),
        )
        ...
        if condition is None:
            ...
            for step in range(self.step_size):
                ...
                theta = (
                    theta
                    + (
                        self.drift_fn(input=theta, t=t)
                        - (self.diffusion_fn(t=t)) ** 2
                        * self.score_estimator(
                            input=theta, condition=condition, times=t
                        )
                    )
                    * delta_t
                    + self.diffusion_fn(t=t)
                    * torch.randn(sample_shape + self._event_shape)
                    * delta_t
                )
    ...
    def sample_with_ode(self, sample_shape, condition):
        ...
        theta0 = self.noise_distribution.sample(sample_shape=sample_shape)
    ...
    def sample(self, sample_shape: torch.Size, condition) -> torch.Tensor:
        if self.condition is None:
            raise ValueError(
                "sampling from the posterior is only possible when specifying a \
                condition"
            )
        if self.sample_with == "sde":
            return self.sample_with_sde(
                sample_shape=sample_shape, condition=self.condition
            )
        if self.sample_with == "ode":
            return self.sample_with_ode(
                sample_shape=sample_shape, condition=self.condition
            )
