...
from torch.dist import Distribution
...
class FlowMatchingEstimator(VectorFieldEstimator):
    ...
    def __init__(
        ...
        weight_fn: Callable = lambda t: 1.0,
        source_distribution: Distribution = torch.distributions.Normal(0, 1),
    ) -> None:
        ...
        Args:
            ...
            mean_fn: Mean function of the flow matching vector field,specifying the mean
                     of the target distribution at a given time. Does specify the mean
                     of the the Gaussian conditional probability flow.
            ...
            source_distribution: _description_. Defaults to
        """
    ...
    def mean_fn_grad(
        self, xs_source: Tensor, xs_target: Tensor, times: Tensor
    ) -> Tensor:
        r"""Time derivative of the mean function. Required for the loss function.

        Note: This method can be implemented by the user, but is not required.
        ...
        with torch.enable_grad():
            ...
            grad = torch.autograd.grad(
                m_t, times, grad_outputs=torch.ones_like(m_t), create_graph=True
            )[0]
        return grad
    ...
    def std_fn_grad(
        ...
        with torch.enable_grad():
            ...
            grad = torch.autograd.grad(
                s_t, times, grad_outputs=torch.ones_like(s_t), create_graph=True
            )[0]
        return grad
    ...
    def loss(self, input: Tensor, condition: Tensor) -> Tensor:
        ...
        u_t = self.std_fn_grad(xs_source, xs_target, times) * eps + self.mean_fn_grad(
            xs_source, xs_target, times
        )
...
class OTFlowMatchingEstimator(FlowMatchingEstimator):
    ...
    def __init__(
        self, net: nn.Module, condition_shape: torch.Size, sigma_min: float = 1e-3
    ):
        super().__init__(net, condition_shape)
